...
- [ ] 1. Project setup and shared domain types
- [ ] 1.1 Initialize the TypeScript project with ESM modules, all runtime and dev dependencies, and build/test tooling
...
- [ ] 1.2 Define the shared domain types and constants used across all components
...
- [ ] 2.1 Define all database tables using Drizzle ORM schema-as-code
...
- [ ] 2.2 Set up the database connection layer and generate the initial migration
...
- [ ] 3. Markdown artifact parser
- [ ] 3.1 (P) Build the core parser that extracts structured data from Markdown artifact files
...
- [ ] 3.2 Add link reference extraction and inline acceptance criteria parsing
...
- [ ] 3.3 Handle artifact type inference and non-artifact file graceful skipping
...
- [ ]* 3.4 Write unit tests for the parser covering all 16 artifact types and edge cases
...
- [ ] 4. Git diff detector
- [ ] 4.1 (P) Implement diff-based file resolution for incremental sync
...
- [ ] 4.2 Implement full tree walk for complete repository scanning
...
- [ ] 5.1 (P) Implement per-artifact validation rules for data integrity checking
...
- [ ] 5.2 Add batch validation for duplicate stable ID detection across a sync run
...
- [ ] 6. Upsert writer
- [ ] 6.1 (P) Implement artifact upsert with conflict resolution on stable ID
...
- [ ] 6.2 Implement soft-delete for removed artifacts and provenance tracking
...
- [ ] 7. Sync engine orchestration
- [ ] 7.1 Wire the full sync pipeline within a single database transaction
...
- [ ] 7.2 Add dependency-order processing and per-file error isolation
...
- [ ] 7.3 Record sync run results with accurate counts and error details
...
- [ ] 8. CLI tool (reqsync)
- [ ] 8.1 (P) Implement the sync full and sync diff commands with required global options
...
- [ ] 8.2 Implement the validate and status commands
...
- [ ] 9. Webhook server
- [ ] 9.1 (P) Implement the webhook endpoint with branch filtering and async sync dispatch
...
- [ ] 9.2 Implement the status endpoint and repository management
...
- [ ] 10. Test repository generator (reqgen)
- [ ] 10.1 (P) Build the core artifact generation engine with templates and deterministic output
...
- [ ] 10.2 (P) Build the traceability link graph for parent-child and cross-cutting relationships
...
- [ ] 10.3 Build multi-commit git history simulating realistic project evolution
...
- [ ] 10.4 Add the CLI interface, malformed file injection, and auxiliary output generators
...
- [ ] 11. Normalized artifact links (stretch)
- [ ] 11.1 Implement second-pass link resolution to populate the normalized links table
...
- [ ] 11.2 Add dangling reference detection and warning logging
...
- [ ] 12.1 (P) Write integration tests for full sync, diff sync, and idempotency
...
- [ ] 12.2 (P) Write integration tests for error isolation and webhook dispatch
...
- [ ] 12.3 (P) Write E2E tests for CLI workflows and generator determinism
